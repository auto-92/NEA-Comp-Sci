...
vehicle_body = Actor('cartoon_van_body')
...
#        truck.y += 10     #moves truck down if collides with track
    ...
    vehicle_body.center = (((wheel_front.x + wheel_back.x)/2), (((wheel_front.y + wheel_back.y)/2)-40))
...
#    clock.schedule(animate(keep_wheels_on_track()), 10.0)
    ...
    vehicle_body.angle = ((180*atan(((wheel_front.y - wheel_back.y)/(wheel_back.x - wheel_front.x))))/math.pi)
    colours[0] = (colours[0] + 1.5) % 256
    colours[1] = (colours[1] + 3) % 256
    colours[2] = (colours[2] +0.5) % 256
...
def play_level_complete_once():
    global played
    if track.right < 1000 and played == False:
...
def wheels_x_position():
    ...
    if wheel_front.y == wheel_back.y:
        ...
        wheel_front_x_position = wheel_back.x + sqrt(((145.0)**2.0) - ((wheel_front.center[1] - wheel_back.center[1])**2.0))
    elif (wheel_back.center[1] - wheel_front.center[1]) > 0.0:
        wheel_front_x_position = wheel_back.x + sqrt(((145.0)**2.0) - ((wheel_back.center[1] - wheel_front.center[1])**2.0))
...
def print_distance_between_wheels_1():
    if (wheel_front.center[1] - wheel_back.center[1]) > 0:
        print(sqrt(((wheel_front.center[1] - wheel_back.center[1])**2) + ((wheel_front.center[0] - wheel_back.center[0])**2)))

def print_distance_between_wheels_2():
    if (wheel_back.center[1] - wheel_front.center[1]) > 0:
        print(sqrt(((wheel_back.center[1] - wheel_front.center[1])**2) + ((wheel_front.center[0] - wheel_back.center[0])**2)))
