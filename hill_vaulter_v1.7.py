...
vehicle_body = Actor('fast_car_body_small', anchor=((wheel_back.center)))

#creates the track and positions it
track = Actor('green_track_2')
track.pos = 0,0
...
backgrounds = ['sky_background', 'red_sky_background', 'milkyway_background']
...
#    screen.fill((0, 50, 200))
    ...
    vehicle_body.draw()
#    truck.draw()
#    print(screen.surface.get_at((79,499)))
...
def move_front_wheel_up():
    ...
    while (colour_wheel_front_tyre_bottom == (42, 149, 14, 255)) or (colour_wheel_front_tyre_bottom == (106, 79, 13, 255)):  
        wheel_front.y -= 2
        colour_wheel_front_tyre_bottom = screen.surface.get_at((int(wheel_front.center[0]),int(wheel_front.center[1]+55)))
def move_back_wheel_up():    
    ...
    while (colour_wheel_back_tyre_bottom == (42, 149, 14, 255)) or (colour_wheel_back_tyre_bottom == (106, 79, 13, 255)):
        colour_wheel_back_tyre_bottom = screen.surface.get_at((int(wheel_back.center[0]),int(wheel_back.center[1]+55)))
        wheel_back.y -= 2
def move_front_wheel_down():
    colour_wheel_front_tyre_bottom = screen.surface.get_at((int(wheel_front.center[0]),int(wheel_front.center[1]+55)))
    while (colour_wheel_front_tyre_bottom != (42, 149, 14, 255)) and (colour_wheel_front_tyre_bottom != (106, 79, 13, 255)):
        wheel_front.y += 2
        colour_wheel_front_tyre_bottom = screen.surface.get_at((int(wheel_front.center[0]),int(wheel_front.center[1]+55)))
def move_back_wheel_down():
    colour_wheel_back_tyre_bottom = screen.surface.get_at((int(wheel_back.center[0]),int(wheel_back.center[1]+55)))
    while (colour_wheel_back_tyre_bottom != (42, 149, 14, 255)) and (colour_wheel_back_tyre_bottom != (106, 79, 13, 255)):
        wheel_back.y += 2
        colour_wheel_back_tyre_bottom = screen.surface.get_at((int(wheel_back.center[0]),int(wheel_back.center[1]+55)))
