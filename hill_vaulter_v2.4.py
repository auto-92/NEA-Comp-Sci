...
vehicle_body = Actor('cartoon_van_body_sharper')
vehicle_body.midbottom = 172, 250

#creates the track and positions it
track = Actor('track_2_green')
track.pos = 0,0
...
#        screen.draw.text("Text color", (50, 30), color=(0,0,200,0))
        screen.draw.text("Hill Vaulter", center = (500, 30), fontname='faster-one-regular', fontsize=60)
...
#draws an intro screen before the game, gives instructions etc..
def draw_intro():
    screen.blit('beige_background', (0,0))
    screen.draw.text("Hill Vaulter", center = (500, 30), fontname='faster-one-regular', fontsize=75)
    intro_text = 'Press space to play.'
    screen.draw.text(intro_text, center = (500, 475), fontname='ostrich-regular', fontsize=45, width = 900,
                     shadow=(1,1), scolor="#202020")
    screen.draw.text("Can you get Barry's old truck over the hills to the shop?", midtop = (500, 390),
                     fontname='walterturncoat', fontsize=30, width=950, ocolor=(255,0,0), owidth=0.5)
...
#used to play a 2-second chime which sounds when you complete a level
...
#rewind the track back to start quickly while 'R' is held down
def rewind_track():
    if keyboard.r and track.right < 9900:
        track.right += 50

#keeps the distance between the front and back wheels the same, so that the wheels stay with the vehicle body
...
#4 functions below to keep the wheels on the track, and stop them going below it
...
#makes the track actor 'scroll' across the screen when right key pressed
