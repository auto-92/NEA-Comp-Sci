...
wheel_front = Actor('car_wheel_smooth')
wheel_front.center = 375,20

#creates a back wheel and positions it
wheel_back = Actor('car_wheel_smooth')
...
#function to give me a value for the height of an actor
def get_actor_height(actor):
    return (actor.midbottom[1] - actor.midtop[1])

#function to give me a value for the width of an actor
def get_actor_width(actor):
    return (actor.midright[0] - actor.midleft[0])

wheel_height = get_actor_height(wheel_front)
wheel_width = get_actor_width(wheel_front)
print(wheel_height//2)
...
def move_front_wheel_up():
    colour_wheel_front_tyre_bottom = screen.surface.get_at((int(wheel_front.center[0]),int(wheel_front.center[1]) + int((wheel_height//2)) + 12))
    while colour_wheel_front_tyre_bottom != (231, 199, 84, 255):  
        wheel_front.y -= 2
        colour_wheel_front_tyre_bottom = screen.surface.get_at((int(wheel_front.center[0]),int(wheel_front.center[1]) + int((wheel_height//2)) + 12))
def move_back_wheel_up():    
    colour_wheel_back_tyre_bottom = screen.surface.get_at((int(wheel_back.center[0]),int(wheel_back.center[1]) + int((wheel_height//2)) + 12))
...
#print(get_actor_height(wheel_front))
#print(get_actor_width(wheel_back))